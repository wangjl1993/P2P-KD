...
def get_linea_lr(epoch, schedule_list, lr):
    if epoch in schedule_list:
        return lr*0.1
    else:
        return lr
...
# SGDR更新学习率
def get_sgdr_lr(epoch, T_0=5, T_mult=2, eta_max=0.1, eta_min=0.):
...
def compute_acc_fgt(acc_dict, num_task):
    avg_acc = sum(acc_dict['task'][1:])/len(acc_dict['task'][1:])
    fgt = []
    for i in range(num_task-1):
        fgt += [max(0, acc_dict[i][0]-j) for j in acc_dict[i][1:]]
    fgt = sum(fgt)/len(fgt)
